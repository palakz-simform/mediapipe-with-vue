...
export class FrameVTO {
  ...
  init = () => {
    ...
    document
      .getElementById(this.threejs_container)
  ...
  updateSize = () => {
    if (
      this.canvasElement.clientWidth > 0 &&
      this.canvasElement.clientHeight > 0
    ) {
      const FOV = this.getFOV();
      this.threeJSCamera = new THREE.PerspectiveCamera(
        FOV,
      ...
      console.log(
        `Updated size: ${this.canvasElement.clientWidth} x ${this.canvasElement.clientHeight}`
      );
    } else {
  ...
  renderTryOn = () => {
    if (this.isTryOn) {
      ...
      if (this.vtoTransformedLeft === null)
        nullProperties.push("vtoTransformedLeft");
      if (this.vtoTransformedRight === null)
        nullProperties.push("vtoTransformedRight");

      if (nullProperties.length === 0) {
        // Position
        // this.glassesMesh.position.set((this.vtoNormalizedX*(1.120)), this.vtoNormalizedY / 2.1, 0);
        const { yaw, pitch, roll } = this.headPose;
        this.glassesMesh.position.set(
          this.vtoNormalizedX,
          this.vtoNormalizedY / 2 - 0.05,
          0.05
        );
  ...
  onResults = (results) => {
    ...
    if (results.multiFaceLandmarks.length > 0) {
      for (const landmarks of results.multiFaceLandmarks) {
        ...
        const leftIrisRelativeDistance = this.calculateDistance(
          landmarks[this.leftInnerIrisEdge],
          landmarks[this.leftOuterIrisEdge]
        );
        // calculate the relative distances for the RIGHT iris diameter
        const rightIrisRelativeDistance = this.calculateDistance(
          landmarks[this.rightInnerIrisEdge],
          landmarks[this.rightOuterIrisEdge]
        );

        // draw green landmarks on iris
        draw.drawConnectors(this.canvasCtx, landmarks, FACEMESH_RIGHT_IRIS, {
          color: "#30FF30",
          lineWidth: 0.25,
        });
        draw.drawConnectors(this.canvasCtx, landmarks, FACEMESH_LEFT_IRIS, {
          color: "#30FF30",
          lineWidth: 0.25,
        });
        ...
        const cheek_x_l =
          landmarks[this.leftTemple].x * this.canvasElement.width;
        const cheek_y_l =
          landmarks[this.leftTemple].y * this.canvasElement.height;
        const cheek_x_r =
          landmarks[this.rightTemple].x * this.canvasElement.width;
        const cheek_y_r =
          landmarks[this.rightTemple].y * this.canvasElement.height;
        ...
        if (this.showEyeBridgeMid === true) {
          ...
          let noseBridgeMid = this.findMidPoint(
            noseBridgeLeft,
            noseBridgeRight
          );
        ...
        this.irisConstant_max = Math.max(
          this.irisConstant_left,
          this.irisConstant_right
        );
        this.irisConstant_min = Math.min(
          this.irisConstant_left,
          this.irisConstant_right
        );
        this.irisConstant_avg =
          (this.irisConstant_left, this.irisConstant_right) / 2;

        // let bridgeMid = this.findMidPoint(landmarks[this.leftIris], landmarks[this.rightIris]);
        let bridgeMid = this.findMidPoint(
          landmarks[this.leftIris],
          landmarks[this.rightIris]
        );
        const irisRelativeDistanceLeft = this.calculateDistance(
          landmarks[this.leftIris],
          bridgeMid,
          false
        );
        const irisRelativeDistanceRight = this.calculateDistance(
          landmarks[this.rightIris],
          bridgeMid,
          false
        );
        ...
        if (
          document.getElementsByClassName(this.output_selector_pd)[0] &&
          this.getFaceShape === true
        ) {
          document.getElementsByClassName(
            this.output_selector_pd
          )[0].innerHTML =
            this.detectedPD.toFixed(1) +
        ...
        if (document.getElementsByClassName(this.output_selector_pd_r)[0]) {
          document.getElementsByClassName(
            ...
            this.detectedPD_R.toFixed(1) +
            ' <small class="text-muted">mm</small>';
        }

        // frame width
        let templeMidPoint = this.findMidPoint(
          landmarks[this.leftTemple],
          landmarks[this.rightTemple]
        );
        let detectedWidth_L =
        ...
        if (document.getElementsByClassName(this.output_selector_width)[0]) {
          document.getElementsByClassName(
            ...
            this.detectedWidth.toFixed(1) +
            ' <small class="text-muted">mm</small>';
        }
        if (
          document.getElementsByClassName(this.output_selector_size)[0] &&
          this.getFaceShape === true
        ) {
        ...
        // vto rotation
        this.vtoRotationX = this.angleFromXAxis(
          landmarks[this.leftTemple],
          landmarks[this.rightTemple],
          this.xDampingFactor
        );
        this.vtoRotationZ = this.angleFromZAxis(
          landmarks[this.leftTemple],
          landmarks[this.rightTemple],
          this.zDampingFactor
        );
        this.vtoRotationY = this.angleFromYAxis(
          landmarks[this.leftTemple],
          landmarks[this.rightTemple],
          this.yDampingFactor
        );
        if (document.getElementsByClassName(this.output_selector_rotation)[0]) {
          document.getElementsByClassName(
            this.output_selector_rotation
          )[0].innerHTML = `${this.vtoRotationX.toFixed(
            1
          )} / ${this.vtoRotationZ.toFixed(
            1
          )} <small class="text-muted">rad</small>`;
        }
        //vto x/y (starts at left top)
        this.vtoX =
          landmarks[this.vtoLeftTopLandmark].x * this.canvasElement.width;
        this.vtoY =
          landmarks[this.vtoLeftTopLandmark].y * this.canvasElement.height;
        this.vtoZ =
          1 /
          (Math.abs(landmarks[this.vtoLeftTopLandmark].z) *
            this.irisConstant_avg);
        ...
        this.vtoNormalizedY = -(
          (this.vtoY / this.canvasElement.height) * 2 -
          1
        );
        ...
        const normalizedMidTemple = {
          x:
            (((templeMidPoint.x * this.canvasElement.width) /
              this.canvasElement.width) *
              2 -
              1) *
            this.vtoAspectRatio,
          y:
            -(
              (templeMidPoint.y * this.canvasElement.height) /
              this.canvasElement.height
            ) *
              2 -
            1,
        ...
        this.vtoWidth =
          this.calculateDistance(
            normalizedLeftTemple,
            normalizedMidTemple,
            false
          ) +
          this.calculateDistance(
            normalizedRightTemple,
            normalizedMidTemple,
            false
          );
        ...
        // if (this.isOriented) {
        this.detectedPD_L = this.addDataPointSize(
          this.rollingAverage,
          "pd_l",
          pd_left,
          500
        );
        this.detectedPD_R = this.addDataPointSize(
          this.rollingAverage,
          "pd_r",
          pd_right,
          500
        );
        this.detectedPD = this.addDataPointSize(
          this.rollingAverage,
          "pd",
          (pd_left + pd_right) / 2,
          500
        );
        this.detectedWidth = this.addDataPointSize(
          this.rollingAverage,
          "width",
  ...
  vtoStart = (imageUrl) => {
    ...
    img.onload = () => {
      ...
      this.glassesGeometry = new THREE.PlaneGeometry(
        this.glassesWidth,
        this.glassesHeight
      ); // Adjust size as needed
      this.glassesMesh = new THREE.Mesh(
  ...
  determineFaceShape = (landmarks) => {
    ...
    const cheekboneWidth = this.euclideanDistance(
      landmarks[93],
      landmarks[323]
    ); // High cheekbones
  ...
  getFOV = () => {
    ...
    const scalingFactor =
      calculateScalingFactor(width) / (this.isModal ? 1 : 1);
